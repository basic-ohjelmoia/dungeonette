 ...
 * encapsulated.
 *
 * The class contains program logic which should be refactored into separate
 * classes.
 *
...
public class Floor {
    ...
    private int maxNumberOfPassages = 200;
    private int plusThese;
    public int level;

    /**
     * Constructor for the floor. Note that the object is constructed WITHOUT
     * the pointOfEntry object.
     *
    ...
    /**
     * "Salt" refers to coarse grid locations where no rooms can be placed. The
     * density of salt has a huge impact on the dungeon's looks. Passageways ARE
     * legal on salted grids.
     *
     */
    ...
    /**
     * Removes all salts from the floor. Used in debugging, might have further
     * uses as well.
     */
    ...
    /**
     * Adds a random passageway to the dungeon. The passageway always is
     * connected FROM an existing room. If the connecting parameter is FALSE,
     * then the passageway is generated as a deadend.
     *
     * @param connecting true if the route connects to another room, false if
     * the route is a deadend
     */
    public void addRandomRoute(boolean connecting) {
        if (routes < this.maxNumberOfPassages) {
            Random randomi = spec.randomi;
            Room origin = null;
            ...
            if (connecting) {
                ...
                while (true) {
                    ...
                    toX = Math.max(1, toX);
                    toX = Math.min(toX, spec.maxX - 2);
                    toY = Math.max(1, toY);
                    toY = Math.min(toY, spec.maxY - 2);
     ...
     * The crossing passageway should result as a dead-end (terminate before
     * breaching any room outer boundary walls). Crossy passage tiles are marked
     * as ¤+ in the dungeon printouts.
     *
     * @param start starting coordinates of the crossed passageway
     * @param dir direction of the MAIN passageway (not the crossing
     * passageway!!!!!)
     */
...
//        routeIDFrom[routes]=-11111;
    ...
    /**
     * If the tile is identifiable to specific room, this matrix stores it's
     * number. Zeroes refer to tiles which DON'T belong to rooms.
     *
    ...
    /**
     * Routes are identified by origin room and destination room id numbers This
     * method returns an array id numbers of the room from which the psassageway
     * originates from.
     *
     * @return id numbers
     */
    ...
    /**
     * Routes are identified by origin room and destination room id numbers This
     * method returns an array id numbers of the room for which the psassageway
     * destines to.
     *
     * @return id numbers
     */
    ...
    /**
     * A placeholderish method which returns a char array containg door
     * locations
     *
    ...
    /**
     * Sets the point of entry (the first room) of the floor. After the point is
     * set, the salt is added.
     *
